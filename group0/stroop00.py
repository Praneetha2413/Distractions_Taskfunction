...
# Distractions
distraction_audio_file = "sound.mp3"
notification_audio_file = "notif_sound.mp3"
notification_image_file = "notif_image.png"
...
def display_color_word():
    ...
    if current_trial < total_trials:
        ...
        if(current_trial == 0):
            nextpromt_label.config(text=f"Focus on COLOR for Trial 1-10\n\n\n Up Next: TRIAL {current_trial + 1} ")
        elif(current_trial == 10):
            nextpromt_label.config(text=f"Focus on WORD for Trial 11-20\n\n\n Up Next: TRIAL {current_trial + 1} ")
        else:
            nextpromt_label.config(text=f"\n\n\n Up Next: TRIAL {current_trial + 1}")

        root.update()
        if(current_trial == 0 or current_trial == 10):
            time.sleep(5)
        else:
            time.sleep(1.5)
...
color_label = tk.Label(root, text='', font=('Helvetica', 40), justify='center')
color_label.pack()

# Create a label to display the prompt
prompt_label = tk.Label(root, text='', font=('Helvetica', 25), fg='black')
prompt_label.pack()
